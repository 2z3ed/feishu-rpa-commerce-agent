...
def test_action_executed_detail_contains_yingdao_bridge_fields(monkeypatch):
    ...
    try:
        ...
        assert "page_url=http://127.0.0.1:8000/api/v1/internal/rpa-sandbox/admin-like/inventory" in detail
        assert "page_profile=internal_inventory_admin_like_v1" in detail
        assert "page_steps=open_dashboard|navigate_inventory_adjust|search_sku|open_drawer|submit|read_page_echo" in detail
    finally:
        db.close()
